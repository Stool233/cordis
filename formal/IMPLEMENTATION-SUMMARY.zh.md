...
> 当前分支角色：`research/paper-trace-baseline` 保留 Cordis `8cc9e33` 的运行时逻辑，只加入观测点、确定性轨迹、论文驱动的 TLA+ 规格和预期失败检查。`yarn formal:baseline` 只有在普通行为检查与 TLC 都复现固定 mismatch 集合时才成功；通过修复后的研究分支是 `research/paper-conformance`，不含插桩的上游补丁分支是 `fix/paper-conformance`。

本文面向希望快速理解本次工作的读者，说明我们为什么要做形式化验证、三个仓库分别发生了什么变化、验证结果意味着什么，以及它不意味着什么。完整的可执行规格仍以本目录中的 TLA+ 模块为准。

## 一句话结论

我们已经为 Cordis 论文建立了一条可重复运行的三层证据链：TLC 对论文抽象状态机做有界穷举；确定性轨迹把上游 Cordis 和 DeepSeek Harness 中的 vendored Cordis 映射到同一抽象状态机并逐状态比较；无法从有限运行轨迹推出的前提则单独审计。本分支保留原始实现以重现偏差，不在插桩提交中修改运行时逻辑。
