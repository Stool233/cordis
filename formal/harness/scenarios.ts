...
const captureOnly = process.env.CORDIS_FORMAL_CAPTURE_ONLY === '1'
const verify = {
  equal(actual: unknown, expected: unknown) {
    if (!captureOnly) assert.equal(actual, expected)
  },
  deepEqual(actual: unknown, expected: unknown) {
    if (!captureOnly) assert.deepEqual(actual, expected)
  },
  async rejects(value: Promise<unknown>) {
    if (captureOnly) {
      try {
        await value
      } catch {}
...
export const scenarios: ScenarioDefinition[] = [
  {
    ...
    async run(_implementation, recorder) {
      ...
      if (captureOnly) recorder.resetTraceWindow()
      await disposal
      verify.deepEqual(order, ['consumer', 'provider'])
    },
  },
  {
    ...
    async run(_implementation, recorder) {
      ...
      if (captureOnly) recorder.resetTraceWindow()
      await disposal
      verify.deepEqual(observed, [true])
      verify.equal(resource.available, false)
    },
  },
  {
    ...
    async run(_implementation, recorder) {
      ...
      await Promise.resolve()
      await Promise.resolve()
      verify.equal(resource.available, true)
      releaseCleanup()
      await disposing
      verify.deepEqual(observed, [true])
      verify.equal(resource.available, false)
  ...
  },
  {
    ...
    async run(_implementation, recorder) {
      ...
      await consumer
      verify.equal(seen.length, 2)
      verify.equal(seen[0], seen[1])
      ...
      verify.deepEqual(providers, ['provider#1', 'provider#2'])
  ...
  },
  {
    ...
    async run(_implementation, recorder) {
      ...
      await unloading
      await consumer
      verify.equal(consumer.state, recorder.fiberState.ACTIVE)
    },
  },
  {
    ...
    async run(_implementation, recorder) {
      ...
      await fiber.dispose()
      verify.deepEqual(restored, [3, 2, 1])
    },
  },
  {
    ...
    async run(_implementation, recorder) {
      ...
      await fiber.dispose()
      verify.deepEqual(restored, ['second:start', 'first:start', 'second:end', 'first:end'])
      verify.equal(maximum, 2)
    },
  },
  {
    ...
    async run({ FiberState }, recorder) {
      ...
      await verify.rejects(failing.await())
      verify.deepEqual(restored, [1])
      verify.equal(sibling.state, FiberState.ACTIVE)
    },
  },
  {
    ...
    async run({ FiberState }, recorder) {
      ...
      verify.equal(child.state, FiberState.DISPOSED)
    },
  },
  {
