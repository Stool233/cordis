...
## Baseline counterexamples and refinement boundary

This branch retains the baseline runtime behavior so its counterexamples remain reproducible. The trace-plus-fix branch corrects the deviations without weakening the paper properties:

1. Provider inverses begin before asynchronous dependents finish unloading. Focused post-activation trace windows reach `inverse-started` without hiding the earlier lifecycle mismatch.
...
3. Reload commits its store while the fiber still projects as Inactive, and an active target can change before L-Leave. Full trace windows reject these landings before later actions are consumed.

`formal:baseline` locks the ordinary behavior failures and the TLC mismatch set as expected failures. A changed or unexpectedly passing baseline fails this diagnostic check.
